...
impl eframe::App for App {
  fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
          ...
          egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            ui.collapsing("Dataset", |ui| {
              ...
              if self.dataset.len() > 0
              {
                ...
                egui::ComboBox::from_label("Column X")
                  .selected_text(format!("{} ({})", self.dataset[0][self.dataset_column_x], self.dataset_column_x))
                  .show_ui(ui, |ui| {
                    for n in 0..column_len {
                      ui.selectable_value(&mut self.dataset_column_x, n, format!("{} ({})", self.dataset[0][n], n));
                    }
                  });

                egui::ComboBox::from_label("Column Y")
                  .selected_text(format!("{} ({})", self.dataset[0][self.dataset_column_y], self.dataset_column_y))
                  .show_ui(ui, |ui| {
                    for n in 0..column_len {
                      ui.selectable_value(&mut self.dataset_column_y, n, format!("{} ({})", self.dataset[0][n], n));
                    }
                  });
              }
            });
            ...
            ui.collapsing("Parameters", |ui| {
              egui::Grid::new("parameter_grid").show(ui, |ui| {
                ...
                egui::ComboBox::from_label("")
                  .selected_text(centroid_placement_methods[self.centroid_placement_method])
                  .show_ui(ui, |ui| {
                    for n in 0..2 {
                      ui.selectable_value(&mut self.centroid_placement_method, n, centroid_placement_methods[n]);
                    }
                  });
                ui.end_row();
              });
            ...
            if self.dataset.len() > 0 {
              if ui.button("Cluster Reset").clicked() {
                ...
                self.cluster.data = (start_index..self.dataset.len()).map(|i| {
                  Vector2 {
                    x: self.dataset[i][self.dataset_column_x].parse::<f64>().unwrap(),
                    y: self.dataset[i][self.dataset_column_y].parse::<f64>().unwrap(),
                  }
                }).collect();
              ...
              if ui.button("Fit Auto").clicked() {
                self.cluster.fit(self.cluster_max_loop, self.cluster_goal_diff);
              }
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
      ...
      let centroids: PlotPoints = (0..centroid_len).map(|i| {
        [self.cluster.centroids[i].x, self.cluster.centroids[i].y]
      }).collect();
      ...
      Plot::new("main_plot")
        .show(ui, |plot_ui| {
          for n in 0..centroid_len {
            let plot_points: PlotPoints = (0..data[n].len()).map(|i| {
              [data[n][i].x, data[n][i].y]
            }).collect();
